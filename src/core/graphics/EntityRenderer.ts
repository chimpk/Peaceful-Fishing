...
export const drawFishTexture = (
  ...
  ctx.save();
  if (customPos) {
    ctx.translate(customPos.x, customPos.y); ctx.rotate(customPos.angle);
    if (customPos.direction === -1) ctx.scale(-1, 1);
    if (isStruggling || isCaught) ctx.translate(-size * 1.0, 0);
  }
  if (isCaught || isGolden) {
    ctx.shadowBlur = isGolden ? 35 : 30; ctx.shadowColor = '#fbbf24';
    ctx.save(); ctx.globalAlpha = 0.3 + Math.sin(frameCount * 0.2) * 0.2; ctx.fillStyle = '#fbbf24'; ctx.beginPath(); ctx.ellipse(0, 0, size * 1.4, size * 0.9, 0, 0, Math.PI * 2); ctx.fill(); ctx.restore();
  } else {
    if (rarity === Rarity.MYTHIC || rarity === Rarity.LEGENDARY) {
        ctx.shadowBlur = rarity === Rarity.MYTHIC ? 25 : 15; ctx.shadowColor = rarity === Rarity.MYTHIC ? '#ff0000' : '#fbbf24';
